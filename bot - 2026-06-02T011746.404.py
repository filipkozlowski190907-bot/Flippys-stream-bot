...
async def check_tiktok_live():
    """Check if the TikTok user is currently live."""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
                    ...
                    return status == 2, live_room  # status 2 = live
...
async def send_live_notification(channel, live_room):
    ...
    embed.set_footer(text="TikTok Live", icon_url="https://raw.githubusercontent.com/filipkozlowski190907-bot/pppp/main/icon.png")
...
async def monitor_tiktok():
    """Background task to monitor TikTok live status."""
    global is_live
    ...
    for guild in client.guilds:
        for ch in guild.text_channels:
            if ch.name == CHANNEL_NAME or CHANNEL_NAME in ch.name:
                channel = ch
                break
        if channel:
            break
